...
# Biến toàn cục lưu ID thư đã gửi lần cuối
last_sent_email_id = {}
...
def check_latest_email(account):
    ...
    try:
        ...
        # Kiểm tra nếu thư mới là thư khác với thư đã gửi lần cuối
        if latest_email_id != last_sent_email_id.get(account["email"]):
            ...
            if email_details:
                ...
                # Cập nhật ID của thư đã gửi
                last_sent_email_id[account["email"]] = latest_email_id
        else:
            print(f"[{account['email']}] Thư mới đã được gửi trước đó.")
    except Exception as e:
